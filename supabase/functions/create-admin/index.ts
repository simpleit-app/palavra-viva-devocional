...
serve(async (req) => {
  ...
  try {
    ...
    if (!authUser?.user) {
      ...
      if (updateError) {
        log("WARNING: Failed to update admin password", { error: updateError.message });
        // Continue anyway, as we still want to ensure the profile and subscription are set up
      } else {
    ...
    return new Response(JSON.stringify({ 
      success: true, 
      message: "Admin user created/updated successfully with Pro access"
    }), {
      headers: { ...corsHeaders, "Content-Type": "application/json" },
      status: 200,
    });
